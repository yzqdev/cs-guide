...
# 关闭电子钱包
用了kde以后，每次打开浏览器都会跳出电子钱包等，影响使用体验
于是就想办法把他关掉！
...
~/.bashrc: 该文件包含专用于当前用户的 bash shell 的配置信息,当登录时以及每次打开新的shell时,该该文件被读取.
...
# 如果需要长久的使用淘宝源则需要如下操作
# 确认当前使用的 Shell 类型（通常为 bash 或 zsh）
# 如果是 bash 则键入 
...
#7. ALTER USER 'root'@'localhost' IDENTIFIED BY '123456';# 然后退出mysql登陆即可
```
...
其中，我将原本的en_US改为了zh_CN，未见异常。如需撤销，仅需将`'/var/lib/postgres/data'`下面的内容清空。
启动/开机启动 PostgreSQL：
...
> 提示： 若创建一个与系统用户同名的数据库用户，并允许其访问 PostgreSQL 数据库，则在登录 PostgreSQL 数据库 shell 时无需切换用户（这样做会比较方便）。
