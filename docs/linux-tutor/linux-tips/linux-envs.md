...
1. 直接运行export命令定义变量
在shell的命令行下直接使用[export 变量名=变量值] 定义变量。该变量只在当前的shell（BASH）或其子shell（BASH）下是有效的，shell关闭了，变量也就失效了，再打开新shell时就没有这个变量，如需使用，则需要重新定义。
1. 修改系统环境变量
系统环境变量一般保存在下面的文件中
...
可自行添加指定的路径，中间用冒号隔开。环境变量更改后，在用户下次登陆时生效，如果想立刻生效，则可执行下面的语句：
...
当通过控制台登录（输入用户名和密码）时，初始化命令行提示符会执行 `.bash_profile` 来配置 Shell 环境。但是如果已经登录到机器，在Gnome或者是KDE也开了一个新的终端窗口（xterm），这时，.bashrc会在窗口命令行提示符出现前被执行。在终端中执行 `/bin/bash` 时，.bashrc 会在新的 bash 实例启动时执行。
