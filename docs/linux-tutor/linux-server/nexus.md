...
- Nexus 安装
  ...
  - 本文习惯 `/opt` 目录下创建一个目录 `setups` 用来存放各种软件安装包；在 `/usr` 目录下创建一个 `program` 用来存放各种解压后的软件包，下面的讲解也都是基于此习惯
  - 已配置第三方源 `EPEL` 和 `RepoForge`。若 `yum install` 安装失败，则可能缺少相关源，请参考源设置相关文章
  - 压缩包下载（由于国内网络的原因不排除你下载不了）：`wget http://download.sonatype.com/nexus/oss/nexus-2.12.0-01-bundle.tar.gz`
  - 如果地址下载不了，因需 VPN 才能访问，你也可以选择降低要求下载 2.11.4-01 版本：<http://pan.baidu.com/s/1mgSNJtA>
    ...
    - 由于目录 `sonatype-work` 以后是做仓库用的，会存储很多 jar，所以这个目录一定要放在磁盘空间大的区内，目前我们还没第一次启动 Nexus，所以这里还是空文件
      - 本文习惯把这类目录放在 `/opt` 下，所以你要特别注意，下面有内容对这个文件夹进行操作的都是基于 opt 目录的：`mv /opt/setup/sonatype-work/ /opt/`
    ...
    - 若 JDK 等系统变量已设置正确，则无需编辑 Nexus 的配置文件，此处列出配置文件路径：`vim /usr/program/nexus2.11.4/bin/jsw/conf/wrapper.conf`
    - 开放防火墙端口：
...
- 下载远程中央库的索引到服务器
  ...
  - 如上图 gif 所示，创建任务开始进行索引下载。需要特别提醒的是，若私服运行在虚拟机上，需确保分配的硬盘空间充足，确保硬盘空间充足，例如分配 10 G（现在还剩下 1.9 G），结果报：**设备上没有空间**
