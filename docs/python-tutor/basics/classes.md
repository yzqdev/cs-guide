...
在了解元类之前，先进一步理解 Python 中的类。在大多数编程语言中，类就是一组用来描述如何生成一个对象的代码段。在 Python 中这一点也是一样的。

在之前学习类的章节也强调过了，下面通过例子回忆一下：
...
但是，Python 中的类有一点跟大多数编程语言不同，在 Python 中，可以把类理解成也是一种对象。对的，这里没有写错，就是对象。

为什么呢？因为只要使用关键字 `class`，Python 解释器在执行的时候就会创建一个对象。
...
当程序运行这段代码的时候，就会在内存中创建一个对象，名字就是 ObjectCreator。这个对象（类）自身拥有创建对象（类实例）的能力，而这就是为什么它是一个类的原因。但是，它的本质仍然是一个对象，于是可以对它做如下的操作：
...
通过上面的介绍，终于带到元类这里来了。创建类的时候，大多数是为了创建类的实例对象。那么元类呢？**元类就是用来创建类的。也可以换个理解方式：元类就是类的类。**

通过上面 `type()` 函数的介绍，知道可以通过 `type()` 函数创建类：
...
到现在，已经知道元类是什么了。那么从始至终还不知道元类到底有什么用。

在了解它有什么用之前，先来了解下怎么自定义元类。因为只有了解了怎么自定义，才能更好地理解它的作用。

首先来了解下 `__metaclass__` 属性。

metaclass，直译为元类，简单的解释就是：

当定义了类以后，就可以根据这个类创建出实例，所以：先定义类，然后创建实例。

但是如果要创建出类呢？那必须根据 metaclass 创建出类，所以：先定义 metaclass，然后创建类。
...
当写下 `class MyObject(object)`，但是类对象 MyObject 还没有在内存中创建。Python 会在类的定义中寻找 `__metaclass__` 属性，如果找到了，Python 就会用它来创建类 MyObject，如果没有找到，就会用内建的 type 函数来创建这个类。如果还不怎么理解，看下下面的流程图：
...
终于到了使用元类了。可是一般来说，根本就用不上它，就像 Python 界的领袖 Tim Peters 说的：
