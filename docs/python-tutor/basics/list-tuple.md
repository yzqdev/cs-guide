...
之前学习了字符串、整数、浮点数几种基本数据类型，现在接着学习两种新的数据类型：列表（List）和元组（tuple）。
...
那为什么要有 List（列表）呢？用一个例子来说明。

现在有一个团队要出去玩，要先报名。如果用之前学过的知识，那么就是用一个字符串变量把他们都记录下来。

但是这样太麻烦了，而且也不美观。

在编程中，一定要避免「重复性工作」。如果有一百个成员，复制粘贴也会很繁琐。

这时候就可以使用列表了。一行代码就可以存放 N 多个名字了。

## 2、怎么创建 List（列表）

从上面的例子可以分析出，列表的格式如下：列表就是用中括号 `[]` 括起来的数据，里面的每一个数据就叫做元素，每个元素之间使用逗号分隔。
...
就像一开始的例子，有时候不需要把全部人员的姓名都打印出来，有时候需要知道第 3 个报名的人是谁，前两名报名的是谁。那么怎么从列表中取出来呢？可以通过列表的下标索引来访问列表中的值，同样也可以使用方括号的形式截取字符。
...
可见，需要知道 `name` 这个列表中第三个报名的是谁，只需要用 `name[2]` 就可以了。这里会问，为什么是 2，不是 3 呢？这是因为在编程世界中，都是从 0 开始的，而不是生活习惯中从 1 开始。所以需要知道第三个，那就是 `name[2]` 就可以了。

从例子来看，还把 `name[0:2]` 的结果打印出来了。从打印结果来看，只打印了第一、第二个元素内容。这里可能会有疑问：为什么不是打印前三个，2 不就是第 3 个吗？那是因为这是**左闭右开**区间的。所以 `name[0:2]` 的意思就是从第 0 个开始取，取到第 2 个，但是不包含第 2 个。

为了更好理解，可以多去尝试下面的各种方式：

看看输出的结果，根据输出的结果和上面讲到的知识，就很容易理解其中的一些用法了。

## 4、如何更新 List（列表）

还是一开始的例子，用代码记录了报名人的名字，后面可能会有新人加入，也有可能发现一开始写错名字了，想要修改。
...
## 5、如何删除 List（列表）里面的元素

那既然这样，肯定会有人中途退出，那么就需要在列表中把他的名字去掉。这时候使用 `del` 语句来删除列表的元素。
...
上一节说了有序列表 List，现在说另一种有序列表叫元组：tuple。

tuple 和 List 非常类似，但是 tuple 一旦初始化就不能修改。也就是说元组（tuple）是不可变的，那么不可变是指什么意思呢？

元组（tuple）不可变是指当创建了 tuple 后，它就不能改变了，也就是说它也没有 `append()`、`insert()` 这样的方法，但它也有获取某个索引值的方法，但是不能赋值。

那么为什么要有 tuple 呢？因为 tuple 是不可变的，所以代码更安全。所以建议能用 tuple 代替 list 就尽量用 tuple。
...
如果不加逗号，创建出来的就不是元组（tuple），而是 `123` 这个数了。这是因为括号 `()` 既可以表示元组（tuple），又可以表示数学公式中的小括号，这就产生了歧义。

所以如果只有一个元素时，不加逗号，计算机就没法识别是进行整数或小数运算还是表示元组。

因此，Python 规定，这种情况下，按小括号进行计算，计算结果自然是 `123`，而如果要表示元组，就需要加个逗号。
...
可能有人会有疑问：上面不是说了 tuple 是不可变的吗？这里怎么又来修改元组了。

那是因为元组中的元素值是不允许修改的，但可以对元组进行连接组合，还有通过修改其他列表的值从而影响 tuple 的值。
...
tuple 元组中的元素值是不允许删除的，但可以使用 `del` 语句来删除整个元组。
...
最后跟列表一样，来一个实例，大家可以多尝试，去把元组的各种玩法玩一遍。
