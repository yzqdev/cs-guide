...
| 概念 | 说明 | 示例 |
|------|------|------|
| **可迭代对象** | 实现了 `__iter__()` 的对象 | `list`, `tuple`, `str`, `dict` |
| **迭代器** | 实现了 `__next__()` 的对象 | `iter([1,2,3])` 的返回值 |
| **生成器** | 用 `yield` 创建的迭代器 | 生成器函数、生成器表达式 |
| **yield** | 暂停函数并返回值 | `def gen(): yield 1` |
| **yield from** | 委托给子生成器（Python 3.3+） | `yield from sub_gen()` |
