...
</code></pre><p>Store 容纳了 state，并且能随时访问 state 的值，那么可以写下如下代码</p><pre><code class="language-javascript">class Store {
...
</code></pre><p>接下来我们考虑 dispatch 逻辑。首先 dispatch 应该接收一个 action 参数，并且发送给 reducer 更新 state。然后如果用户 subscribe 了 state，我们还应该调用函数，那么可以写下如下代码</p><pre><code class="language-javascript">dispatch(action) {
    this._state = this.reducer(this.state, action)
    this.subscribers.forEach(fn =&gt; fn(this.getState()))
}

</code></pre><p>reducer 逻辑很简单，在 constructor 时将 reducer 保存起来即可，那么可以写下如下代码</p><pre><code class="language-javascript">constructor(initState, reducer) {
    this._state = initState
    this._reducer = reducer
}

</code></pre><p>现在一个 Redux 的简易半成品已经完成了，我们可以来执行下以下代码</p><pre><code class="language-javascript">const initState = {value: 0}
...
</code></pre><p>最后一步让我们来完成 subscribe 函数， subscribe 函数调用如下</p><pre><code class="language-javascript">store.subscribe(() =&gt;
  console.log(store.getState())
)

</code></pre><p>所以 subscribe 函数应该接收一个函数参数，将该函数参数 push 进数组中，并且调用该函数</p><pre><code class="language-javascript">subscribe(fn) {
