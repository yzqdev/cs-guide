...
</code></pre><p>可以看到，这里使用了两者中不同的方式定义类，可以看到最大的不同就是，新式类继承了<code>object</code> 类，在 Python2 中，我们定义类的时候最好定义新式类，当然在 Python3 中不存在这个问题了，因为 Python3 中所有类都是新式类。</p><p>那么新式类和旧式类有什么区别呢？</p><p>运行下下面的那段代码：</p><pre><code class="language-python">#!/usr/bin/env python
# -*- coding: UTF-8 -*-
...
</code></pre><p>这是 python 2.7 运行的结果：</p><pre><code>/Users/twowater/dev/python/test/venv/bin/python /Users/twowater/dev/python/test/com/twowater/test.py
...
</code></pre><p>这是 Python 3.6 运行的结果：</p><pre><code>/usr/local/bin/python3.6 /Users/twowater/dev/python/test/com/twowater/test.py
...
</code></pre><p>仔细观察输出的结果，对比一下，就能观察出来，注意喔，Pyhton3 中输出的结果是一模一样的，因为Python3 中没有新式类旧式类的问题。</p><h1 id="七、类的继承" tabindex="-1"><a class="header-anchor" href="#七、类的继承"><span>七、类的继承</span></a></h1><h2 id="_1、定义类的继承" tabindex="-1"><a class="header-anchor" href="#_1、定义类的继承"><span>1、定义类的继承</span></a></h2><p>说到继承，你一定会联想到继承你老爸的家产之类的。</p><p>类的继承也是一样。</p><p>比如有一个旧类，是可以算平均数的。然后这时候有一个新类，也要用到算平均数，那么这时候我们就可以使用继承的方式。新类继承旧类，这样子新类也就有这个功能了。</p><p>通常情况下，我们叫旧类为父类，新类为子类。</p><p>首先我们来看下类的继承的基本语法：</p><pre><code class="language-python">class ClassName(BaseClassName):
    &lt;statement-1&gt;
    .
    .
    .
    &lt;statement-N&gt;
</code></pre><p>在定义类的时候，可以在括号里写继承的类，如果不用继承类的时候，也要写继承 object 类，因为在 Python 中 object 类是一切类的父类。</p><p>当然上面的是单继承，Python 也是支持多继承的，具体的语法如下：</p><pre><code class="language-python">class ClassName(Base1,Base2,Base3):
    &lt;statement-1&gt;
    .
    .
    .
    &lt;statement-N&gt;
</code></pre><p>多继承有一点需要注意的：若是父类中有相同的方法名，而在子类使用时未指定，python 在圆括号中父类的顺序，从左至右搜索 ， 即方法在子类中未找到时，从左到右查找父类中是否包含方法。</p><p>那么继承的子类可以干什么呢？</p><p>继承的子类的好处：</p><ul><li>会继承父类的属性和方法</li><li>可以自己定义，覆盖父类的属性和方法</li></ul><h2 id="_2、调用父类的方法" tabindex="-1"><a class="header-anchor" href="#_2、调用父类的方法"><span>2、调用父类的方法</span></a></h2><p>一个类继承了父类后，可以直接调用父类的方法的，比如下面的例子，<code>UserInfo2</code> 继承自父类 <code>UserInfo</code> ，可以直接调用父类的 <code>get_account</code> 方法。</p><pre><code class="language-python">#!/usr/bin/env python
# -*- coding: UTF-8 -*-
...
</code></pre><h2 id="_3、父类方法的重写" tabindex="-1"><a class="header-anchor" href="#_3、父类方法的重写"><span>3、父类方法的重写</span></a></h2><p>当然，也可以重写父类的方法。</p><p>示例：</p><pre><code class="language-python">#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
...
</code></pre><p>输出的结果如下：</p><pre><code class="language-txt">True
True
True
True
True
False
</code></pre><p>可以看到 <code>isinstance()</code> 不仅可以告诉我们，一个对象是否是某种类型，也可以用于基本类型的判断。</p><h1 id="八、类的多态" tabindex="-1"><a class="header-anchor" href="#八、类的多态"><span>八、类的多态</span></a></h1><p>多态的概念其实不难理解，它是指对不同类型的变量进行相同的操作，它会根据对象（或类）类型的不同而表现出不同的行为。</p><p>事实上，我们经常用到多态的性质，比如：</p><pre><code>&gt;&gt;&gt; 1 + 2
3
&gt;&gt;&gt; &#39;a&#39; + &#39;b&#39;
&#39;ab&#39;
</code></pre><p>可以看到，我们对两个整数进行 + 操作，会返回它们的和，对两个字符进行相同的 + 操作，会返回拼接后的字符串。</p><p>也就是说，不同类型的对象对同一消息会作出不同的响应。</p><p>看下面的实例，来了解多态：</p><pre><code class="language-python">#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
