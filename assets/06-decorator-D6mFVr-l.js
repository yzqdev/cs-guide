import{i as e,r as t,s as n,t as r}from"./app-Bmf6MlmU.js";var i=JSON.parse(`{"path":"/python-tutor/basics/06-decorator.html","title":"装饰器","lang":"zh-CN","frontmatter":{"order":6,"description":"装饰器 上一篇文章通过解决一个需求问题了解了闭包，本文也将通过逐步演变一个需求，一步一步来了解 Python 装饰器。 首先有一个输出员工打卡信息的函数： 输出的结果如下： 然后，产品反馈需要加上打卡的具体日期。直接添加打印日期的代码如下： 输出结果如下： 这样改是可以，但是改变了函数的功能结构。这个函数定义的时候就是打印员工信息和提示打卡成功，现在增...","head":[["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"装饰器\\",\\"image\\":[\\"\\"],\\"dateModified\\":\\"2026-07-16T05:29:15.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"yzqdev\\",\\"url\\":\\"http://www.yzqdev.top\\"}]}"],["meta",{"property":"og:url","content":"https://yzqdev.github.io/cs-guide/cs-guide/python-tutor/basics/06-decorator.html"}],["meta",{"property":"og:site_name","content":"cs-guide"}],["meta",{"property":"og:title","content":"装饰器"}],["meta",{"property":"og:description","content":"装饰器 上一篇文章通过解决一个需求问题了解了闭包，本文也将通过逐步演变一个需求，一步一步来了解 Python 装饰器。 首先有一个输出员工打卡信息的函数： 输出的结果如下： 然后，产品反馈需要加上打卡的具体日期。直接添加打印日期的代码如下： 输出结果如下： 这样改是可以，但是改变了函数的功能结构。这个函数定义的时候就是打印员工信息和提示打卡成功，现在增..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2026-07-16T05:29:15.000Z"}],["meta",{"property":"article:modified_time","content":"2026-07-16T05:29:15.000Z"}]]},"git":{"createdTime":1653497324000,"updatedTime":1784179755000,"contributors":[{"name":"yzqdev","username":"yzqdev","email":"yzqdev@outlook.com","commits":7,"url":"https://github.com/yzqdev"}]},"readingTime":{"minutes":4.84,"words":1453},"filePathRelative":"python-tutor/basics/06-decorator.md","autoDesc":true}`),a={name:`06-decorator.md`};function o(r,i,a,o,s,c){return n(),t(`div`,null,[...i[0]||=[e(`<h1 id="装饰器" tabindex="-1"><a class="header-anchor" href="#装饰器"><span>装饰器</span></a></h1><p>上一篇文章通过解决一个需求问题了解了闭包，本文也将通过逐步演变一个需求，一步一步来了解 Python 装饰器。</p><p>首先有一个输出员工打卡信息的函数：</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">def</span> <span class="token function">punch</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
...
<span class="line"></span></code></pre></div><p>然后，产品反馈需要加上打卡的具体日期。直接添加打印日期的代码如下：</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">import</span> time</span>
<span class="line"></span>
<span class="line"></span>
...
<span class="line"></span></code></pre></div><p>这样改是可以，但是改变了函数的功能结构。这个函数定义的时候就是打印员工信息和提示打卡成功，现在增加打印日期的代码，可能会造成代码重复的问题。比如还有一个地方只需要打印员工信息和打卡成功就行了，不需要日期，那么就需要重写一个函数。打印当前日期的功能方法是经常使用的，可以作为公共函数给各个模块方法调用。</p><p>既然是这样，可以使用函数式编程来修改这部分代码。通过之前的学习，知道 Python 函数有两个特点：函数也是一个对象，而且函数里可以嵌套函数。那么修改一下代码变成下面这个样子：</p><div class="language-python" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">import</span> time</span>
<span class="line"></span>
<span class="line"></span>
<span class="line"><span class="token keyword">def</span> <span class="token function">punch</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    <span class="token keyword">print</span><span class="token punctuation">(</span><span class="token string">&#39;昵称：两点水  部门：做鸭事业部 上班打卡成功&#39;</span><span class="token punctuation">)</span></span>
...
<span class="line">昵称：两点水  部门：做鸭事业部 上班打卡成功</span>
<span class="line"></span></code></pre></div><p>这样是不是发现，这样就没有改动 <code>punch</code> 方法，而且任何需要打印当前日期的函数都可以把函数传入 <code>add_time</code> 就可以了，例如：</p><div class="language-python line-numbers-mode" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">import</span> time</span>
<span class="line"></span>
<span class="line"></span>
<span class="line"><span class="token keyword">def</span> <span class="token function">punch</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    <span class="token keyword">print</span><span class="token punctuation">(</span><span class="token string">&#39;昵称：两点水  部门：做鸭事业部 上班打卡成功&#39;</span><span class="token punctuation">)</span></span>
...
<span class="line"></span></code></pre></div><p>使用函数式编程是不是很方便，但是每次调用时都需要把原来的函数作为参数传递进去，能否有更好的实现方式呢？有的，就是本文要介绍的装饰器。装饰器的写法跟闭包是差不多的，不过没有了自由变量。那么这里直接给出上面那段代码的装饰器写法，来对比一下装饰器写法和函数式编程有什么不同。</p><div class="language-python line-numbers-mode" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">import</span> time</span>
<span class="line"></span>
<span class="line"></span>
...
<span class="line">昵称：两点水  部门：做鸭事业部 上班打卡成功</span>
<span class="line"></span></code></pre></div><p>通过代码，通过代码，可以知道装饰器函数一般做这三件事：</p><ol><li>接收一个函数作为参数</li><li>嵌套一个包装函数，包装函数会接收原函数的相同参数，并执行原函数，且还会执行附加功能</li><li>返回嵌套函数</li></ol><p>可是，装饰器的写法看起来比函数式编程还复杂。</p><p>那是因为还没有用到装饰器的&quot;语法糖&quot;。Python 在引入装饰器(Decorator)的时候，没有引入任何新的语法特性，都是基于函数的语法特性。这也就说明了装饰器不是 Python 特有的，而是每个语言通用的一种编程思想。只不过 Python 设计出了 <code>@</code> 语法糖，让定义装饰器、把装饰器调用原函数再把结果赋值为原函数的对象名的过程变得更加简单、方便，所以 Python 装饰器的核心可以说就是它的语法糖。 那么如何使用它的语法糖呢？很简单，根据上面的写法写完装饰器函数后，直接在原来的函数上加 <code>@</code> 和装饰器的函数名。如下：</p><div class="language-python line-numbers-mode" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">import</span> time</span>
<span class="line"></span>
<span class="line"></span>
<span class="line"><span class="token keyword">def</span> <span class="token function">decorator</span><span class="token punctuation">(</span>func<span class="token punctuation">)</span><span class="token punctuation">:</span></span>
<span class="line">    <span class="token keyword">def</span> <span class="token function">punch</span><span class="token punctuation">(</span><span class="token punctuation">)</span><span class="token punctuation">:</span></span>
...
<span class="line">昵称：两点水  部门：做鸭事业部 上班打卡成功</span>
<span class="line"></span></code></pre></div><p>这样就方便很多了。使用了装饰器后，直接在原本的函数上加上装饰器的语法糖即可，本函数无需任何改变，调用的地方也不需修改。</p><p>不过这里一直有个问题，输出打卡信息的是固定的，如果需要通过参数来传递，装饰器该怎么写呢？装饰器中的函数可以使用 <code>*args</code> 可变参数，可是仅仅使用 <code>*args</code> 不能完全包括所有参数的情况，比如关键字参数就不能了。为了兼容关键字参数，还需要加上 <code>**kwargs</code>。</p><p>因此，装饰器的最终形式可以写成这样：</p><div class="language-python line-numbers-mode" data-highlighter="prismjs" data-ext="py"><pre><code class="language-python"><span class="line"><span class="token keyword">import</span> time</span>
<span class="line"></span>
<span class="line"></span>
<span class="line"><span class="token keyword">def</span> <span class="token function">decorator</span><span class="token punctuation">(</span>func<span class="token punctuation">)</span><span class="token punctuation">:</span></span>
...
<span class="line"></span></code></pre></div>`,36)]])}var s=r(a,[[`render`,o]]);export{i as _pageData,s as default};