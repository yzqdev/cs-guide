...
<span class="line"></span></code></pre></div><ul><li>to：导航路径，要填写的是你在router/index.js文件里配置的path值，如果要导航到默认首页，只需要写成 to=&quot;/&quot; ，</li><li>[text] ：就是我们要显示给用户的导航名称。 <strong>2.router-view</strong> 用于渲染匹配到的组件。 ①.可以给router-view组件设置<strong>transition过渡</strong>（<a href="https://link.jianshu.com/?t=http%3A%2F%2Fhyuhan.com%2F2016%2F12%2F08%2Fvue-transition%2F" target="_blank" rel="noopener noreferrer">Vue2.0 Transition常见用法全解惑</a>）。</li></ul><div class="language-jsscript" data-highlighter="prismjs" data-ext="jsscript" data-title="jsscript"><pre><code><span class="line">&lt;transition name=&quot;fade&quot;&gt;</span>
...
<span class="line"></span></code></pre></div><h2 id="四、动态路由匹配" tabindex="-1"><a class="header-anchor" href="#四、动态路由匹配"><span>四、动态路由匹配</span></a></h2><p>我们经常需要把某种模式匹配到的所有路由，全都映射到同个组件。 调用router的map方法映射路由，每条路由以key-value的形式存在，key是路径，value是组件。例如：&#39;/home&#39;是一条路由的key，它表示路径；{component: Home}则表示该条路由映射的组件：</p><div class="language-javascript" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line">router<span class="token punctuation">.</span><span class="token function">map</span><span class="token punctuation">(</span><span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre></div><p>例如，我们有一个 User 组件，对于所有 ID 各不相同的用户，都要使用这个组件来渲染。那么，我们可以在 vue-router 的路由路径中使用『动态路径参数』来达到这个效果。</p><div class="language-javascript" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line"><span class="token keyword">const</span> User <span class="token operator">=</span> <span class="token punctuation">{</span></span>
...
<span class="line">  <span class="token punctuation">]</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p>例如： /user/foo 和 /user/bar 都将映射到相同的路由。 一个『路径参数』使用冒号 : 标记。当匹配到一个路由时，参数值会被设置到<strong>this.$route.params</strong> 中。例如： <img src="`+t+'" alt="1624847415596-3352a92f-4311-4d2e-bcd4-e48721bfebe3.png"></p><h2 id="五、vue-router参数传递" tabindex="-1"><a class="header-anchor" href="#五、vue-router参数传递"><span>五、vue-router参数传递</span></a></h2><p>1.用name传值（不推荐） <img src="'+l+`" alt="1624847415714-529369b2-6c50-47af-93cb-3368824f70b4.png"> 2.通过 标签中的to传参</p><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">&lt;router-link :to=&quot;{name:‘dxl’,params:{key:value}}&quot;&gt;东西里&lt;/router-link&gt;</span>
<span class="line"></span></code></pre></div><ul><li>name：就是我们在路由配置文件中起的name值。 另：<strong>命名路由</strong>就是用一个名称来标识一个路由，在定义路由的时候设置一个name属性即可。<strong>在router-link中也可以用路由的名字来链接到一个路由。</strong></li><li>params：就是我们要传的参数，它也是对象形势，在对象里可以传递多个值。 最后用<code>$route.params.username</code>进行接收. 3.用url传参 上面第五点也有提到。 <strong>:冒号的形式传递参数</strong> (1).在router路由配置文件里以冒号的形式设置参数</li></ul><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">{</span>
<span class="line">    path:&#39;/params/:newsId/:userName,</span>
<span class="line">    component:Params</span>
<span class="line">}</span>
<span class="line"></span></code></pre></div><p>(2).组件形式，在src/components目录下建立我们params.vue组件。 我们在页面里输出了url传递的参数。</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">&lt;template&gt;</span>
...
<span class="line">}</span>
<span class="line">&lt;/script&gt;</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>(3).标签path路径中传值</p><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">&lt;router-link to=&quot;/params/888/dxl shuai&gt;params&lt;/router-link&gt;</span>
<span class="line"></span></code></pre></div><p>(4).正则表达式在URL传值中的应用 希望我们传递的新闻ID只能是数字的形式，这时候我们就需要在传递时有个基本的类型判断，<code>path:&#39;/params/:newsId(\\\\d+)/:userName&#39;</code></p><h2 id="六-响应路由参数的变化" tabindex="-1"><a class="header-anchor" href="#六-响应路由参数的变化"><span>六.响应路由参数的变化</span></a></h2><p>当使用路由参数时，例如从 /user/foo 导航到 /user/bar，原来的组件实例会被复用。因为两个路由都渲染同个组件，比起销毁再创建，复用则显得更加高效。不过，这也意味着组件的生命周期钩子不会再被调用。 复用组件时，想对路由参数的变化作出响应的话: (1). watch（监测变化） $route 对象：</p><div class="language-jsscript" data-highlighter="prismjs" data-ext="jsscript" data-title="jsscript"><pre><code><span class="line">const User = {</span>
<span class="line">  template: &#39;...&#39;,</span>
...
<span class="line">  }</span>
<span class="line">}</span>
<span class="line"></span></code></pre></div><p>(2).beforeRouteUpdate 导航守卫 如果目的地和当前路由相同，只有参数发生了改变 (比如从一个用户资料到另一个 <code>/users/1</code> -&gt; <code>/users/2</code>)，你需要使用 <code>beforeRouteUpdate</code>来响应这个变化 (比如抓取用户信息)。</p><div class="language-jsscript" data-highlighter="prismjs" data-ext="jsscript" data-title="jsscript"><pre><code><span class="line">const User = {</span>
<span class="line">  template: &#39;...&#39;,</span>
...
<span class="line">  }</span>
<span class="line">}</span>
<span class="line"></span></code></pre></div><h2 id="七、实现不同路由不同页面标题" tabindex="-1"><a class="header-anchor" href="#七、实现不同路由不同页面标题"><span>七、实现不同路由不同页面标题</span></a></h2><div class="language-jsscript" data-highlighter="prismjs" data-ext="jsscript" data-title="jsscript"><pre><code><span class="line">// 定义路由的时候如下定义，name也可为中文</span>
...
<span class="line">})</span>
<span class="line"></span></code></pre></div><h2 id="八、重定向" tabindex="-1"><a class="header-anchor" href="#八、重定向"><span>八、重定向</span></a></h2><p>刚进入应用都是进入到“/”这个路由的，如果想直接进入到“/goods”怎么办，有两种方法。一种是利用重定向，另一种是利用vue-router的导航式编程。 redirect基本重定向：</p><div class="language-javascript" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line"><span class="token keyword">const</span> routes <span class="token operator">=</span> <span class="token punctuation">[</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre></div><p>★.这里不得不提到<strong>alias</strong>别名的使用。 1.首先我们在路由配置文件里给路径起一个别名，dxl。</p><div class="language-css" data-highlighter="prismjs" data-ext="css" data-title="css"><pre><code><span class="line"><span class="token punctuation">{</span></span>
...
<span class="line"> <span class="token punctuation">}</span></span>
<span class="line"></span></code></pre></div><p>2.配置我们的，起过别名之后，可以直接使用标签里的to属性，进行重新定向。</p><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">&lt;router-link to=&quot;/dxl&quot;&gt;jspang&lt;/router-link&gt;</span>
<span class="line"></span></code></pre></div><p>区别：</p><ul><li>redirect：仔细观察URL，redirect是直接改变了url的值，把url变成了真实的path路径。</li><li>alias：URL路径没有别改变，这种情况更友好，让用户知道自己访问的路径，只是改变了中的内容。 ★★★.注意一个大坑： <strong>别名alias在path为&#39;/&#39;中，是不起作用的。</strong></li></ul><div class="language-css" data-highlighter="prismjs" data-ext="css" data-title="css"><pre><code><span class="line"><span class="token punctuation">{</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre></div><h2 id="九、编程式导航" tabindex="-1"><a class="header-anchor" href="#九、编程式导航"><span>九、编程式导航</span></a></h2><p><strong>1.router.push( )</strong> 想要导航到不同的 URL，则使用 <strong>router.push (在创建vue实例并挂载后调用)</strong>。router.push方法就是用来动态导航到不同的链接的，这个方法会向 history 栈添加一个新的记录，所以，当用户点击浏览器后退按钮时，则回到之前的 URL。 当你点击  时，这个方法会在内部调用，所以说，点击  等同于调用 router.push(...)。 <img src="`+i+`" alt="1624847415629-4a7a5f1e-7644-4370-9ed7-e1f83ce4873f.png"><strong>注意： 如果提供了 path，params 会被忽略，而 query 并不属于这种情况。取而代之的是下面例子的做法，你需要提供路由的 name 或手写完整的带有参数的 path：</strong></p><div class="language-jsscript" data-highlighter="prismjs" data-ext="jsscript" data-title="jsscript"><pre><code><span class="line">const userId = 123</span>
...
<span class="line"></span></code></pre></div><p><strong>2.router.replace( )</strong> router.replace跟 router.push 很像，唯一的不同就是，它不会向 history 添加新记录，而是跟它的方法名一样 —— 替换掉当前的 history 记录。 <img src="`+r+`" alt="1624847415617-6b0abd5e-0f61-4274-b577-0d5dff873bb1.png"><strong>3.router.go(n)</strong> 这个方法的参数是一个整数，意思是在 history 记录中向前或者后退多少步，类似 window.history.go(n)。</p><div class="language-cpp" data-highlighter="prismjs" data-ext="cpp" data-title="cpp"><pre><code><span class="line"><span class="token comment">// 在浏览器记录中前进一步，等同于 history.forward()</span></span>
...
<span class="line"></span></code></pre></div><p>自定义一个goback()方法，并使用this.$router.go(-1),进行后退操作</p><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">&lt;script&gt;</span>
<span class="line">export default {</span>
...
<span class="line">},</span>
<span class="line"></span></code></pre></div><p>三个参数：</p><ul><li>to:路由将要跳转的路径信息，信息是包含在对像里边的。</li><li>from:路径跳转前的路径信息，也是一个对象的形式。</li><li>next:路由的控制参数，常用的有next(true)和next(false)。 <strong>2.写在模板中的钩子函数：</strong> 写在模板中就可以有两个钩子函数可以使用。 beforeRouteEnter：在路由进入前的钩子函数。 beforeRouteLeave：在路由离开前的钩子函数。</li></ul><div class="language-jsscript line-numbers-mode" data-highlighter="prismjs" data-ext="jsscript" data-title="jsscript"><pre><code><span class="line">export default {</span>
<span class="line">  name: &#39;params&#39;,</span>
<span class="line">  data () {</span>
<span class="line">    return {</span>
<span class="line">      msg: &#39;params page&#39;</span>
...
<span class="line">  <span class="token punctuation">]</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p><strong>使用 props 将组件和路由解耦：</strong></p><div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line"><span class="token keyword">const</span> User <span class="token operator">=</span> <span class="token punctuation">{</span></span>
...
<span class="line">  <span class="token punctuation">]</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>这样你便可以在任何地方使用该组件，使得该组件更易于重用和测试。 <strong>1.布尔模式</strong> 如果 props 被设置为 true，route.params 将会被设置为组件属性。 <strong>2.对象模式</strong> 如果 props 是一个对象，它会被按原样设置为组件属性。当 props 是静态的时候有用。</p><div class="language-javascript" data-highlighter="prismjs" data-ext="js" data-title="js"><pre><code><span class="line"><span class="token keyword">const</span> router <span class="token operator">=</span> <span class="token keyword">new</span> <span class="token class-name">VueRouter</span><span class="token punctuation">(</span><span class="token punctuation">{</span></span>
<span class="line">  <span class="token literal-property property">routes</span><span class="token operator">:</span> <span class="token punctuation">[</span></span>
...
<span class="line">  <span class="token punctuation">]</span></span>
<span class="line"><span class="token punctuation">}</span><span class="token punctuation">)</span></span>
<span class="line"></span></code></pre></div><p><strong>3.函数模式</strong> 你可以创建一个函数返回 props。这样你便可以将参数转换成另一种类型，将静态值与基于路由的值结合等等。</p><div class="language-jsscript" data-highlighter="prismjs" data-ext="jsscript" data-title="jsscript"><pre><code><span class="line">const router = new VueRouter({</span>
<span class="line">  routes: [</span>
