...
</code></pre><p><code>manifest.json</code>定义了插件的目录文件结构，上面配置对应这样的结构:</p><pre><code class="language-text">.
├── assets
│   └── logo.png
├── css
│   └── content.css
...
</code></pre><p>插入步骤为:</p><ol><li>用传入的组件通过<code>extend</code>生成构造器，将实例化后的的<code>vm</code>返回</li><li>遍历目标选择器dom</li><li>通过<code>createElement</code> 生成一个<code>div</code>插入到目标dom上</li><li>调用<code>vm</code>实例<code>$mount</code>挂载目标dom</li></ol><p>接下来把我们的组件插入到页面上:</p><pre><code class="language-js">import App from &quot;./App/App.vue&quot;;
import insert from &quot;@/utils/insert&quot;;

insert(App);

</code></pre><p>上面的插入方法都是通过<code>new Vue</code>的方式生成，那页面上可能会存在多个Vue根实例，组件(除非父子组件)间就不能用<code>props/$emit</code>通信，我们可以引入<code>mixin</code>，配合<code>vuex</code>将<code>store</code>混合到全局<code>Vue</code>上(当然还可以使用<code>event bus</code>)</p><pre><code class="language-js">// store mixin
...
</code></pre><ul><li><code>devtools</code>页面中获取到接口响应实体后再将内容发送出去，具体的模块通信可以看<a href="https://www.cnblogs.com/liuxianan/p/chrome-plugin-develop.html#%E6%B6%88%E6%81%AF%E9%80%9A%E4%BF%A1" target="_blank" rel="noopener noreferrer">这里</a>。 缺点:需要开启<code>F12</code></li><li>重发请求 因为使用插件的用户在目标页处在登录状态，我们就可以利用登录状态(<code>cookie</code>)来拷贝目标接口地址，再通过请求重发获取响应内容，我们可以这样实现:</li></ul><pre><code class="language-js">import axios from &quot;@/utils/axios&quot;;
...
</code></pre><ul><li>缺点:重发请求需要消耗性能</li><li>注入js，替换ajax对象(<strong>推荐</strong>) 笔者遇到的情况非常严苛: <ul><li>插件项目是基于<code>content scripts</code>，<code>devtools</code>方式要打开F12，用户是开发者也许能够理解，但对普通用户肯定会影响到插件使用体验</li><li>使用<code>重发请求</code>方式，但目标网站中的目标接口安全措施做的非常完美：请求url中有一个随机参数，这个参数由<code>鼠标位置</code>、<code>时间戳</code>、<code>页面高度</code>等参数合成，可以说独一无二。虽然在网上找了解出该参数的方法，但重发请求后，返回的内容与原请求响应内容不一致(也就是说该接口的内容是随机返回的)。</li></ul></li></ul><p>前两种方式对笔者的实际情况不适用，笔者从<code>请求拦截</code>到<code>请求替换</code>的思路中找到最终的解决方案。我们可以这样实现:</p><pre><code class="language-js">// inject.js
...
</code></pre><p>这种方式是使用<code>injected-script</code>，原理是先缓存页面中原<code>ajax</code>请求对象，在原<code>ajax</code>对象上添加<code>onload</code>方法，监听请求完成的回调，再将目标接口的响应实体通过相应的通信方法发送出去。 在<code>content scripts</code>中将<code>injected-script</code>插入到页面上</p><pre><code class="language-js">// content.js
injectJS();
