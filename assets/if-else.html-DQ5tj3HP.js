import{_ as e,c as n,o as a,d as p}from"./app-CbULZrmi.js";const t={},i=p(`<h1 id="条件语句" tabindex="-1"><a class="header-anchor" href="#条件语句"><span>条件语句</span></a></h1><p>通常都听到别人说，计算机很牛逼，很聪明，其实计算机一点都不聪明，光是你要跟他沟通，都会气 shi 你，聪明的是在写程序的你。</p><p>写程序就是跟计算机沟通，告诉它要做什么。</p><p>竟然是这样，那么肯定缺少不了一些沟通逻辑。比如你要告诉计算机在什么情况下做什么？或者在哪个时间点做什么？</p><p>这都需要用到逻辑判断。这一章节，主要就是说这个。</p><h1 id="目录" tabindex="-1"><a class="header-anchor" href="#目录"><span>目录</span></a></h1><p><img src="http://twowaterimage.oss-cn-beijing.aliyuncs.com/2019-09-09-条件语句和循环语句.png" alt=""></p><h1 id="一、条件语句" tabindex="-1"><a class="header-anchor" href="#一、条件语句"><span>一、条件语句</span></a></h1><h2 id="_1、什么是条件语句" tabindex="-1"><a class="header-anchor" href="#_1、什么是条件语句"><span>1、什么是条件语句</span></a></h2><p>Python 条件语句跟其他语言基本一致的，都是通过一条或多条语句的执行结果（ True 或者 False ）来决定执行的代码块。</p><p>Python 程序语言指定任何非 0 和非空（null）值为 True，0 或者 null 为 False。</p><p>执行的流程图如下：</p><p><img src="http://upload-images.jianshu.io/upload_images/2136918-4ee2486190450a1a?imageMogr2/auto-orient/strip|imageView2/2/w/1240" alt="if语句流程图"></p><h2 id="_2、if-语句的基本形式" tabindex="-1"><a class="header-anchor" href="#_2、if-语句的基本形式"><span>2、if 语句的基本形式</span></a></h2><p>Python 中，if 语句的基本形式如下：</p><pre><code class="language-python">if 判断条件：
    执行语句……
else：
    执行语句……
</code></pre><p>之前的章节也提到过，Python 语言有着严格的缩进要求，因此这里也需要注意缩进，也不要少写了冒号 <code>:</code> 。</p><p>if 语句的判断条件可以用&gt;（大于）、&lt;(小于)、==（等于）、&gt;=（大于等于）、&lt;=（小于等于）来表示其关系。</p><p>例如：</p><pre><code class="language-python"># -*-coding:utf-8-*-
...
if num :
    ...
    执行语句4……
</code></pre><p>实例：</p><pre><code class="language-python"># -*-coding:utf-8-*-
...
</code></pre><p>输出的结果：</p><pre><code class="language-txt">良好
</code></pre><h2 id="_4、if-语句多个条件同时判断" tabindex="-1"><a class="header-anchor" href="#_4、if-语句多个条件同时判断"><span>4、if 语句多个条件同时判断</span></a></h2><p>有时候我们会遇到多个条件的时候该怎么操作呢？</p><p>比如说要求 java 和 python 的考试成绩要大于 80 分的时候才算优秀，这时候该怎么做？</p><p>这时候我们可以结合 <code>or</code> 和 <code>and</code> 来使用。</p><p>or （或）表示两个条件有一个成立时判断条件成功</p><p>and （与）表示只有两个条件同时成立的情况下，判断条件才成功。</p><p>例如：</p><pre><code class="language-python"># -*-coding:utf-8-*-
...
while i &lt; 10:
    print(i)
    i = i + 1
</code></pre><p>虽然打印的结果是一样的，但是细细品味你会发现，他们执行的顺序和知道的条件是不同的。</p><h2 id="_6、嵌套循环" tabindex="-1"><a class="header-anchor" href="#_6、嵌套循环"><span>6、嵌套循环</span></a></h2><p>循环语句和条件语句一样，都是可以嵌套的。</p><p>具体的语法如下：</p><p><strong>for 循环嵌套语法</strong></p><pre><code class="language-python">for iterating_var in sequence:
   for iterating_var in sequence:
      statements(s)
   statements(s)
</code></pre><p><strong>while 循环嵌套语法</strong></p><pre><code class="language-python">while expression:
   while expression:
      statement(s)
   statement(s)
</code></pre><p>除此之外，你也可以在循环体内嵌入其他的循环体，如在 while 循环中可以嵌入 for 循环， 反之，你可以在 for 循环中嵌入 while 循环</p><p>比如：</p><p>当我们需要判断 sum 大于 1000 的时候，不在相加时，可以用到 break ，退出整个循环。</p><pre><code class="language-python">count = 1
sum = 0
while (count &lt;= 100):
    sum = sum + count
    ...
    count = count + 1
print(sum)
</code></pre><p>输出的结果：</p><pre><code class="language-txt">1035
</code></pre><p>有时候，我们只想统计 1 到 100 之间的奇数和，那么也就是说当 count 是偶数，也就是双数的时候，我们需要跳出当次的循环，不想加，这时候可以用到 break</p><pre><code class="language-python">count = 1
sum = 0
while (count &lt;= 100):
    ...
    count = count + 1
print(sum)
</code></pre><p>输出的语句：</p><pre><code class="language-txt">2500
</code></pre><p>还有：</p><pre><code class="language-python">for num in range(10,20):  # 迭代 10 到 20 之间的数字
   for i in range(2,num): # 根据因子迭代
      ...
      print (&#39;%d 是一个质数&#39; % num)
</code></pre><p>输出的结果：</p><pre><code class="language-txt">10 是一个合数
11 是一个质数
12 是一个合数
13 是一个质数
14 是一个合数
...
18 是一个合数
19 是一个质数
</code></pre><p>当然，这里还用到了 <code>for … else</code> 语句。</p><p>其实 for 循环中的语句和普通的没有区别，else 中的语句会在循环正常执行完（即 for 不是通过 break 跳出而中断的）的情况下执行。</p><p>当然有 <code>for … else</code> ，也会有 <code>while … else</code> 。他们的意思都是一样的。</p><h1 id="三、条件语句和循环语句综合实例" tabindex="-1"><a class="header-anchor" href="#三、条件语句和循环语句综合实例"><span>三、条件语句和循环语句综合实例</span></a></h1><h2 id="_1、打印九九乘法表" tabindex="-1"><a class="header-anchor" href="#_1、打印九九乘法表"><span>1、打印九九乘法表</span></a></h2><pre><code class="language-python"># -*- coding: UTF-8 -*-
...
for i in range(1, 10):
        ...
        print()
</code></pre><p>输出的结果:</p><pre><code class="language-txt">1x1=1   
2x1=2   2x2=4   
3x1=3   3x2=6   3x3=9   
...
</code></pre><h2 id="_2、判断是否是闰年" tabindex="-1"><a class="header-anchor" href="#_2、判断是否是闰年"><span>2、判断是否是闰年</span></a></h2><pre><code class="language-python">
# 判断是否是闰年
