import{_ as e,c as t,o as _,d as n}from"./app-CbULZrmi.js";const d={},o=n(`<h1 id="魔法方法" tabindex="-1"><a class="header-anchor" href="#魔法方法"><span>魔法方法</span></a></h1><p>有时候修改文章，真的修改到想死。真的很耗时间，很烦的。</p><p>好吧，每次都是安慰自己，快完结了，快更新完了。</p><h1 id="目录" tabindex="-1"><a class="header-anchor" href="#目录"><span>目录</span></a></h1><p><img src="http://twowaterimage.oss-cn-beijing.aliyuncs.com/2019-10-14-Python 的 Magic Method.png" alt=""></p><h1 id="一、python-的-magic-method" tabindex="-1"><a class="header-anchor" href="#一、python-的-magic-method"><span>一、Python 的 Magic Method</span></a></h1><p>在 Python 中，所有以 &quot;__&quot; 双下划线包起来的方法，都统称为&quot;魔术方法&quot;。比如我们接触最多的 <code>__init__</code> 。</p><p>魔术方法有什么作用呢？</p><p>使用这些魔术方法，我们可以构造出优美的代码，将复杂的逻辑封装成简单的方法。</p><p>那么一个类中有哪些魔术方法呢？</p><p>我们可以使用 Python 内置的方法 <code>dir()</code> 来列出类中所有的魔术方法.示例如下：</p><pre><code class="language-python">#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
...
if __name__ == &#39;__main__&#39;:
    print(dir(User()))
</code></pre><p>输出的结果：</p><pre><code>[&#39;__class__&#39;, &#39;__delattr__&#39;, &#39;__dict__&#39;, &#39;__dir__&#39;, &#39;__doc__&#39;, &#39;__eq__&#39;, &#39;__format__&#39;, &#39;__ge__&#39;, &#39;__getattribute__&#39;, &#39;__gt__&#39;, &#39;__hash__&#39;, &#39;__init__&#39;, &#39;__init_subclass__&#39;, &#39;__le__&#39;, &#39;__lt__&#39;, &#39;__module__&#39;, &#39;__ne__&#39;, &#39;__new__&#39;, &#39;__reduce__&#39;, &#39;__reduce_ex__&#39;, &#39;__repr__&#39;, &#39;__setattr__&#39;, &#39;__sizeof__&#39;, &#39;__str__&#39;, &#39;__subclasshook__&#39;, &#39;__weakref__&#39;]
</code></pre><p>可以看到，一个类的魔术方法还是挺多的，不过我们只需要了解一些常见和常用的魔术方法就好了。</p><h1 id="二、构造-new-和初始化-init" tabindex="-1"><a class="header-anchor" href="#二、构造-new-和初始化-init"><span>二、构造(<code>__new__</code>)和初始化(<code>__init__</code>)</span></a></h1><p>通过之前的学习，我们已经知道定义一个类时，我们经常会通过 <code>__init__(self)</code> 的方法在实例化对象的时候，对属性进行设置。</p><p>比如下面的例子：</p><pre><code class="language-python">#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
...
user=User(&#39;两点水&#39;,23)
</code></pre><p>实际上，创建一个类的过程是分为两步的，一步是创建类的对象，还有一步就是对类进行初始化。</p><p><code>__new__</code> 是用来创建类并返回这个类的实例, 而<code>__init__</code> 只是将传入的参数来初始化该实例.<code>__new__</code> 在创建一个实例的过程中必定会被调用,但 <code>__init__</code> 就不一定，比如通过 pickle.load 的方式反序列化一个实例时就不会调用 <code>__init__</code> 方法。</p><p><img src="http://twowaterimage.oss-cn-beijing.aliyuncs.com/2019-10-14-Python类创建的过程.png" alt=""></p><p><code>def __new__(cls)</code> 是在 <code>def __init__(self)</code> 方法之前调用的，作用是返回一个实例对象。还有一点需要注意的是：<code>__new__</code> 方法总是需要返回该类的一个实例，而 <code>__init__</code> 不能返回除了 <code>None</code> 的任何值</p><p>具体的示例：</p><pre><code class="language-python">#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
...
if __name__ == &#39;__main__&#39;:
    usr = User(&#39;两点水&#39;, 23)
</code></pre><p>看看输出的结果：</p><pre><code class="language-txt">调用了 def __new__ 方法
(&#39;两点水&#39;, 23)
调用了 def __init__ 方法
</code></pre><p>通过打印的结果来看，我们就可以知道一个类创建的过程是怎样的了，先是调用了 <code>__new__</code> 方法来创建一个对象，把参数传给 <code>__init__</code> 方法进行实例化。</p><p>其实在实际开发中，很少会用到 <code>__new__</code> 方法，除非你希望能够控制类的创建。通常讲到 <code>__new__</code> ，都是牵扯到 <code>metaclass</code>(元类)的。</p><p>当然当一个对象的生命周期结束的时候，析构函数 <code>__del__</code> 方法会被调用。但是这个方法是 Python 自己对对象进行垃圾回收的。</p><h1 id="三、属性的访问控制" tabindex="-1"><a class="header-anchor" href="#三、属性的访问控制"><span>三、属性的访问控制</span></a></h1><p>之前也有讲到过，Python 没有真正意义上的私有属性。然后这就导致了对 Python 类的封装性比较差。我们有时候会希望 Python 能够定义私有属性，然后提供公共可访问的 get 方法和 set 方法。Python 其实可以通过魔术方法来实现封装。</p><table><thead><tr><th>方法</th><th>说明</th></tr></thead><tbody><tr><td><code>__getattr__(self, name)</code></td><td>该方法定义了你试图访问一个不存在的属性时的行为。因此，重载该方法可以实现捕获错误拼写然后进行重定向, 或者对一些废弃的属性进行警告。</td></tr><tr><td><code>__setattr__(self, name, value)</code></td><td>定义了对属性进行赋值和修改操作时的行为。不管对象的某个属性是否存在,都允许为该属性进行赋值.有一点需要注意，实现 <code>__setattr__</code> 时要避免&quot;无限递归&quot;的错误，</td></tr><tr><td><code>__delattr__(self, name)</code></td><td><code>__delattr__</code> 与 <code>__setattr__</code> 很像，只是它定义的是你删除属性时的行为。实现 <code>__delattr__</code> 是同时要避免&quot;无限递归&quot;的错误</td></tr><tr><td><code>__getattribute__(self, name)</code></td><td><code>__getattribute__</code> 定义了你的属性被访问时的行为，相比较，<code>__getattr__</code> 只有该属性不存在时才会起作用。因此，在支持 <code>__getattribute__ </code>的 Python 版本,调用<code>__getattr__</code> 前必定会调用 <code>__getattribute__\`\`__getattribute__</code> 同样要避免&quot;无限递归&quot;的错误。</td></tr></tbody></table><p>通过上面的方法表可以知道，在进行属性访问控制定义的时候你可能会很容易的引起一个错误，可以看看下面的示例：</p><pre><code class="language-python">def __setattr__(self, name, value):
    self.name = value
...
def __setattr__(self, name, value):
    ...
    # 定制特有属性
</code></pre><p>上面方法的调用具体示例如下：</p><pre><code class="language-python">#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
...
</code></pre><p>输出的结果如下：</p><pre><code class="language-txt">获取 name 值
两点水
...
</code></pre><p>通过这个例子，可以很好的观察到这 <code>__get__()</code> 和 <code>__set__()</code> 这些方法的调用。</p><p>再看一个经典的例子</p><p>我们知道，距离既可以用单位&quot;米&quot;表示,也可以用单位&quot;英尺&quot;表示。 现在我们定义一个类来表示距离,它有两个属性: 米和英尺。</p><pre><code class="language-python">#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
...
</code></pre><h1 id="六、运算符相关的魔术方法" tabindex="-1"><a class="header-anchor" href="#六、运算符相关的魔术方法"><span>六、运算符相关的魔术方法</span></a></h1><p>运算符相关的魔术方法实在太多了,j就大概列举下面两类：</p><h2 id="_1、比较运算符" tabindex="-1"><a class="header-anchor" href="#_1、比较运算符"><span>1、比较运算符</span></a></h2><table><thead><tr><th>魔术方法</th><th>说明</th></tr></thead><tbody><tr><td><code>__cmp__(self, other)</code></td><td>如果该方法返回负数，说明 <code>self &lt; other</code>; 返回正数，说明 <code>self &gt; other</code>; 返回 0 说明 <code>self == other </code>。强烈不推荐来定义 <code>__cmp__</code> , 取而代之, 最好分别定义 <code>__lt__</code>, <code>__eq__</code> 等方法从而实现比较功能。 <code>__cmp__</code> 在 Python3 中被废弃了。</td></tr><tr><td><code>__eq__(self, other)</code></td><td>定义了比较操作符 == 的行为</td></tr><tr><td><code>__ne__(self, other)</code></td><td>定义了比较操作符 != 的行为</td></tr><tr><td><code>__lt__(self, other)</code></td><td>定义了比较操作符 &lt; 的行为</td></tr><tr><td><code>__gt__(self, other)</code></td><td>定义了比较操作符 &gt; 的行为</td></tr><tr><td><code>__le__(self, other)</code></td><td>定义了比较操作符 &lt;= 的行为</td></tr><tr><td><code>__ge__(self, other)</code></td><td>定义了比较操作符 &gt;= 的行为</td></tr></tbody></table><p>来看个简单的例子就能理解了：</p><pre><code class="language-python">#!/usr/bin/env python3
# -*- coding: UTF-8 -*-
...
</code></pre><p>输出的结果为：</p><pre><code class="language-txt">__eq__
num1 == num2 ? --------&gt; False
