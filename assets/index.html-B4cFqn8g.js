import{_ as e,c as n,o as a,d as l}from"./app-CbULZrmi.js";const p={},s=l(`<h1 id="一些很有用的shell" tabindex="-1"><a class="header-anchor" href="#一些很有用的shell"><span>一些很有用的shell</span></a></h1><div class="hint-container tip"><p class="hint-container-title">提示</p><p>看到一位大佬介绍shell脚本介绍的非常好,转载来用一下 源地址:<a href="https://mp.weixin.qq.com/s?__biz=MzI0MDQ4MTM5NQ==&amp;mid=2247496756&amp;idx=1&amp;sn=03c2d54ab2ac11ffc58266f964ee9e3a&amp;chksm=e9189128de6f183e74243e993bf0715d8bea5bad56852a4f6d0fe6d884cbe3af07f2a27b01ae&amp;scene=21#wechat_redirect" target="_blank" rel="noopener noreferrer">链接</a></p></div><h2 id="前言" tabindex="-1"><a class="header-anchor" href="#前言"><span>前言</span></a></h2><p>由于工作需要，最近重新开始拾掇shell脚本。虽然绝大部分命令自己平时也经常使用，但是在写成脚本的时候总觉得写的很难看。而且当我在看其他人写的脚本的时候，总觉得难以阅读。毕竟shell脚本这个东西不算是正经的编程语言，他更像是一个工具，用来杂糅不同的程序供我们调用。</p><p>因此很多人在写的时候也是想到哪里写到哪里，基本上都像是一段超长的main函数，不忍直视。同时，由于历史原因，shell有很多不同的版本，而且也有很多有相同功能的命令需要我们进行取舍，以至于代码的规范很难统一。</p><p>考虑到上面的这些原因，我查阅了一些相关的文档，发现这些问题其实很多人都考虑过，而且 也形成了一些不错的文章，但是还是有点零散。因此我就在这里把这些文章稍微整理了一下，作为以后我自己写脚本的技术规范。</p><h2 id="代码风格规范" tabindex="-1"><a class="header-anchor" href="#代码风格规范"><span>代码风格规范</span></a></h2><p>开头有“蛇棒”</p><p>所谓shebang其实就是在很多脚本的第一行出现的以#!开头的注释，他指明了当我们没有指定解释器的时候默认的解释器，一般可能是下面这样：</p><pre><code>#!/bin/bash
</code></pre><p>当然，解释器有很多种，除了bash之外，我们可以用下面的命令查看本机支持的解释器:</p><pre><code>$ cat /etc/shells
...
echo &quot;*.sh&quot;
</code></pre><p>在很多情况下，在将变量作为参数的时候，一定要注意上面这一点，仔细体会其中的差异。上面只是一个非常小的例子，实际应用的时候由于这个细节导致的问题实在是太多了。。。</p><h3 id="巧用main函数" tabindex="-1"><a class="header-anchor" href="#巧用main函数"><span>巧用main函数</span></a></h3><p>我们知道，像java，C这样的编译型语言都会有一个函数入口，这种结构使得代码可读性很强，我们知道哪些直接执行，那些是函数。但是脚本不一样，脚本属于解释性语言，从第一行直接执行到最后一行，如果在这当中命令与函数糅杂在一起，那就非常难读了。</p><p>用python的朋友都知道，一个合乎标准的python脚本大体上至少是这样的：</p><pre><code>#!/usr/bin/env python
...
if __name__==&#39;__main__&#39;:
    func1()
    func2()
</code></pre><p>他用一个很巧妙的方法实现了我们习惯的main函数，使得代码可读性更强。</p><p>在shell中，我们也有类似的小技巧:</p><pre><code>#!/usr/bin/env bash
...
main &quot;$@&quot;
</code></pre><p>我们可以采用这种写法，同样实现类似的main函数，使得脚本的结构化程度更好。</p><h3 id="考虑作用域" tabindex="-1"><a class="header-anchor" href="#考虑作用域"><span>考虑作用域</span></a></h3><p>shell中默认的变量作用域都是全局的，比如下面的脚本：</p><pre><code>#!/usr/bin/env bash
...
func
echo $var
</code></pre><p>他的输出结果就是2而不是1，这样显然不符合我们的编码习惯，很容易造成一些问题。</p><p>因此，相比直接使用全局变量，我们最好使用local readonly这类的命令，其次我们可以使用declare来声明变量。这些方式都比使用全局方式定义要好。</p><h3 id="函数返回值" tabindex="-1"><a class="header-anchor" href="#函数返回值"><span>函数返回值</span></a></h3><p>在使用函数的时候一定要注意，shell中函数的返回值只能是整数，估计是因为一般情况下一个函数的返回值通常表示这个函数的运行状态，所以一般都是0或者是１就够了，因此就设计成了这样。不过，如果非得想传递字符串，也可以通过下面变通的方法:</p><pre><code>func(){
    echo &quot;2333&quot;
}
res=$(func)
echo &quot;This is from $res.&quot;
</code></pre><p>这样，通过echo或者print之类的就可以做到传一些额外参数的目的。</p><h3 id="间接引用值" tabindex="-1"><a class="header-anchor" href="#间接引用值"><span>间接引用值</span></a></h3><p>什么叫间接引用？比如下面这个场景：</p><pre><code>VAR1=&quot;2323232&quot;
VAR2=&quot;VAR1&quot;
</code></pre><p>我们有一个变量VAR1，又有一个变量VAR2，这个VAR2的值是VAR1的名字，那么我们现在想通过VAR2来获取VAR1的值，这时候应该怎么办呢？</p><p>比较土鳖的方法是这样：</p><pre><code>eval echo \\$$VAR2
</code></pre><p>啥意思呢？其实就是构造了一个字符串echo XXX，这个XXX就是XXX”，这个XXX就是VAR2的值VAR1，然后再用eval强制解析，这样就做到了变相取值。</p><p>这个用法的确可行，但是看起来十分的不舒服，很难直观的去理解，我们并不推荐。而且事实上我们本身就不推荐使用eval这个命令。</p><p>比较舒服的写法是下面这样:</p><pre><code>echo \${!VAR1}
</code></pre><p>通过在变量名前加一个!就可以做到简单的间接引用了。</p><p>不过需要注意的是，用上面的方法，我们只能够做到取值，而不能做到赋值。如果想要做到赋值，还要老老实实的用eval来处理:</p><pre><code>VAR1=VAR2
eval $VAR1=233
echo $VAR2
</code></pre><h3 id="巧用heredocs" tabindex="-1"><a class="header-anchor" href="#巧用heredocs"><span>巧用heredocs</span></a></h3><p>所谓heredocs，也可以算是一种多行输入的方法，即在”&lt;&lt;”后定一个标识符，接着我们可以输入多行内容，直到再次遇到标识符为止。</p><p>使用heredocs，我们可以非常方便的生成一些模板文件：</p><pre><code>cat&gt;&gt;/etc/rsyncd.conf &lt;&lt; EOF
log file = /usr/local/logs/rsyncd.log
transfer logging = yes
log format = %t %a %m %f %b
syslog facility = local3
EOF
</code></pre><h3 id="学会查路径" tabindex="-1"><a class="header-anchor" href="#学会查路径"><span>学会查路径</span></a></h3><p>很多情况下，我们会先获取当前脚本的路径，然后一这个路径为基准，去找其他的路径。通常我们是直接用pwd以期获得脚本的路径。</p><p>不过其实这样是不严谨的，pwd获得的是当前shell的执行路径，而不是当前脚本的执行路径。</p><p>正确的做法应该是下面这两种：</p><pre><code>script_dir=$(cd $(dirname $0) &amp;&amp; pwd)
script_dir=$(dirname $(readlink -f $0 ))
</code></pre><p>应当先cd进当前脚本的目录然后再pwd，或者直接读取当前脚本的所在路径。</p><h3 id="代码要简短" tabindex="-1"><a class="header-anchor" href="#代码要简短"><span>代码要简短</span></a></h3><p>这里的简短不单单是指代码长度，而是只用到的命令数。原则上我们应当做到，能一条命令解决的问题绝不用两条命令解决。这不仅牵涉到代码的可读性，而且也关乎代码的执行效率。</p><p>最最经典的例子如下：</p><pre><code>cat /etc/passwd | grep root
grep root /etc/passwd
</code></pre><p>cat命令最为人不齿的用法就是这样，用的没有任何意义，明明一条命令可以解决，他非得加根管道。。。</p><p>其实代码简短在还能某种程度上能保证效率的提升，比如下面的例子：</p><pre><code>#method1
...
</code></pre><p>这两种方法做的事情都一样，就是查找所有的.txt后缀的文件并做一系列替换。前者是多次执行find，后者是执行一次find，但是增加了sed的模式串。第一种更直观一点，但是当替换的量变大的时候，第二种的速度就会比第一种快很多。这里效率提升的原因，就是第二种只要执行一次命令，而第一种要执行多次。并且，巧用xargs命令，我们还可以十分方便的进行并行化处理:</p><pre><code>find . -name &#39;*.txt&#39; |xargs -P $(nproc) sed -i &quot;s/233/666/g;s/235/626/g;s/333/616/g;s/233/664/g&quot;
</code></pre><p>通过-P参数指定并行度，可以进一步加快执行效率。</p><h3 id="命令并行化" tabindex="-1"><a class="header-anchor" href="#命令并行化"><span>命令并行化</span></a></h3><p>当我们需要充分考虑执行效率时，我们可能需要在执行命令的时候考虑并行化。shell中最简单的并行化是通过”&amp;”以及”wait”命令来做:</p><pre><code>func(){
    #do sth
｝
for((i=0;i&lt;10;i++))do
    func &amp;
