import{_ as n,c as e,a,o as i}from"./app-Dd1lYWPM.js";const l={};function d(t,s){return i(),e("div",null,s[0]||(s[0]=[a(`<h1 id="程序调试" tabindex="-1"><a class="header-anchor" href="#程序调试"><span>程序调试</span></a></h1><h2 id="进程调试" tabindex="-1"><a class="header-anchor" href="#进程调试"><span>进程调试</span></a></h2><h3 id="gdb-程序交互调试" tabindex="-1"><a class="header-anchor" href="#gdb-程序交互调试"><span>gdb 程序交互调试</span></a></h3><p>GDB是一个由GNU开源组织发布的、UNIX/LINUX操作系统下的、基于命令行的、功能强大的程序调试工具。</p><p>对于一名Linux下工作的c++程序员，gdb是必不可少的工具；</p><p>GDB中的命令固然很多，但我们只需掌握其中十个左右的命令，就大致可以完成日常的基本的程序调试工作。</p><p>以下从一个完整的调试过程简单说明最基本的几个命令;</p><div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh" data-title="sh"><pre><code><span class="line">    <span class="token variable">$gdb</span> programmer     <span class="token comment"># 启动gdb</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>当你完成了第一个程序调试之后，你当然会需要更多的命令：关于gdb常用命令及各种调试方法详见 <code>gdb</code>{.interpreted-text role=&quot;ref&quot;} ;</p><p>同时，你需要更高效的调试：常用的调试命令都会有单字符的缩写，使用缩写更方便；同时，直接敲回车表示重复执行上一步命令；这在单步调试时非常有用；</p><h3 id="pstack-跟踪栈空间" tabindex="-1"><a class="header-anchor" href="#pstack-跟踪栈空间"><span>pstack 跟踪栈空间</span></a></h3><p>pstack是一个脚本工具，可显示每个进程的栈跟踪。pstack 命令必须由相应进程的属主或 root 运行。其核心实现就是使用了gdb以及thread apply all bt命令;</p><p>语法:</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $pstack &lt;program-pid&gt;</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>示例:</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $ pstack 4551</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="strace-分析系统调用" tabindex="-1"><a class="header-anchor" href="#strace-分析系统调用"><span>strace 分析系统调用</span></a></h3><p>strace常用来跟踪进程执行时的系统调用和所接收的信号。在Linux世界，进程不能直接访问硬件设备，当进程需要访问硬件设备(比如读取磁盘文件，接收网络数据等等)时，必须由用户态模式切换至内核态模式，通过系统调用访问硬件设备。strace可以跟踪到一个进程产生的系统调用,包括参数，返回值，执行消耗的时间。</p><p>完整程序:</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    strace -o output.txt -T -tt -e trace=all -p 28979</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>跟踪28979进程的所有系统调用（-e trace=all），并统计系统调用的花费时间，以及开始时间（以可视化的时分秒格式显示），最后将记录结果存在output.txt文件里面。</p><p>查看进程正在做什么(实时输出进程执行系统调用的情况):</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $strace -p &lt;process-pid&gt;</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>关于strace的详细介绍，详见 <code>strace</code>{.interpreted-text role=&quot;ref&quot;} ;</p><h2 id="目标文件分析" tabindex="-1"><a class="header-anchor" href="#目标文件分析"><span>目标文件分析</span></a></h2><h3 id="nm" tabindex="-1"><a class="header-anchor" href="#nm"><span>nm</span></a></h3><p>nm用来列出目标文件的符号清单。</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $nm myProgrammer</span>
...
<span class="line">    ...</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>每个可执行代码段将在需要特定的事件时执行，这些事件包括库的初始化和该程序本身主入口点。</p><p>对于那些着迷于底层编程细节的程序员来说，这是一个功能非常强大的工具，可用于研究编译器和汇编器的输出。细节信息，比如这段代码中所显示的这些信息，可以揭示有关本地处理器本身运行方式的很多内容。对该处理器制造商提供的技术文档进行深入的研究，您可以收集关于一些有价值的信息，通过这些信息可以深入地了解内部的运行机制，因为功能程序提供了清晰的输出。</p><p>关于objdump工具的参数说明及更多示例详见 <code>objdump</code>{.interpreted-text role=&quot;ref&quot;} ;</p><h3 id="readelf" tabindex="-1"><a class="header-anchor" href="#readelf"><span>readelf</span></a></h3><p>这个工具和objdump命令提供的功能类似，但是它显示的信息更为具体，并且它不依赖BFD库(BFD库是一个GNU项目，它的目标就是希望通过一种统一的接口来处理不同的目标文件）；</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $readelf -all a.out</span>
...
<span class="line">      ...</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>ELF Header 为该文件中所有段入口显示了详细的摘要。在列举出这些 Header 中的内容之前，您可以看到 Header 的具体数目。在研究一个较大的目标文件时，该信息可能非常有用。</p><p>除了所有这些段之外，编译器可以将调试信息放入到目标文件中，并且还可以显示这些信息。输入下面的命令，仔细分析编译器的输出（假设您扮演了调试程序的角色）:</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $readelf --debug-dump a.out | more </span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><p>调试工具，如 GDB，可以读取这些调试信息，并且当程序在调试器中运行的同时，您可以使用该工具显示更具描述性的标记，而不是对代码进行反汇编时的原始地址值。</p><p>关于readelf工具的参数说明及更多示例详见 <code>readelf</code>{.interpreted-text role=&quot;ref&quot;} ;</p><h3 id="size-查看程序内存占用" tabindex="-1"><a class="header-anchor" href="#size-查看程序内存占用"><span>size 查看程序内存占用</span></a></h3><p>size这个工具用来查看程序运行时各个段的实际内存占用:</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $size a.out</span>
<span class="line">    text       data     bss     dec     hex filename</span>
<span class="line">    1146        256       8    1410     582 a.out</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="file-文件类型查询" tabindex="-1"><a class="header-anchor" href="#file-文件类型查询"><span>file 文件类型查询</span></a></h3><p>这个工具用于查看文件的类型；</p><p>比如我们在64位机器上发现了一个32位的库，链接不上，这就有问题了： :</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $file a.out</span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><p>也可以查看Core文件是由哪个程序生成:</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $file core.22355</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><h3 id="strings-查询数据中的文本信息" tabindex="-1"><a class="header-anchor" href="#strings-查询数据中的文本信息"><span>strings 查询数据中的文本信息</span></a></h3><p>一个文件中包含二进制数据和文本数据，如果只需要查看其文本信息，使用这个命令就很方便；过滤掉非字符数据，将文本信息输出:</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $strings &lt;objfile&gt;</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><h3 id="fuser-显示文件使用者" tabindex="-1"><a class="header-anchor" href="#fuser-显示文件使用者"><span>fuser 显示文件使用者</span></a></h3><p>显示所有正在使用着指定的file, file system 或者 sockets的进程信息;</p><pre><code>$fuser -m -u redis-server
...
0000100   ,   i   )   ;  \\n   }  \\n
0000107
</code></pre><p>注：类似命令还有hexdump（十六进制输出）</p>`,84)]))}const c=n(l,[["render",d],["__file","program_debug.html.vue"]]),r=JSON.parse('{"path":"/linux-tutor/common/program_debug.html","title":"程序调试","lang":"zh-CN","frontmatter":{"description":"程序调试 进程调试 gdb 程序交互调试 GDB是一个由GNU开源组织发布的、UNIX/LINUX操作系统下的、基于命令行的、功能强大的程序调试工具。 对于一名Linux下工作的c++程序员，gdb是必不可少的工具； GDB中的命令固然很多，但我们只需掌握其中十个左右的命令，就大致可以完成日常的基本的程序调试工作。 以下从一个完整的调试过程简单说明最基...","head":[["meta",{"property":"og:url","content":"https://yzqdev.github.io/cs-guide/cs-guide/linux-tutor/common/program_debug.html"}],["meta",{"property":"og:site_name","content":"cs-guide"}],["meta",{"property":"og:title","content":"程序调试"}],["meta",{"property":"og:description","content":"程序调试 进程调试 gdb 程序交互调试 GDB是一个由GNU开源组织发布的、UNIX/LINUX操作系统下的、基于命令行的、功能强大的程序调试工具。 对于一名Linux下工作的c++程序员，gdb是必不可少的工具； GDB中的命令固然很多，但我们只需掌握其中十个左右的命令，就大致可以完成日常的基本的程序调试工作。 以下从一个完整的调试过程简单说明最基..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2023-05-14T11:30:49.000Z"}],["meta",{"property":"article:author","content":"yzqdev"}],["meta",{"property":"article:modified_time","content":"2023-05-14T11:30:49.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"程序调试\\",\\"image\\":[\\"\\"],\\"dateModified\\":\\"2023-05-14T11:30:49.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"yzqdev\\",\\"url\\":\\"http://www.yzqdev.top\\"}]}"]]},"headers":[{"level":2,"title":"进程调试","slug":"进程调试","link":"#进程调试","children":[{"level":3,"title":"gdb 程序交互调试","slug":"gdb-程序交互调试","link":"#gdb-程序交互调试","children":[]},{"level":3,"title":"pstack 跟踪栈空间","slug":"pstack-跟踪栈空间","link":"#pstack-跟踪栈空间","children":[]},{"level":3,"title":"strace 分析系统调用","slug":"strace-分析系统调用","link":"#strace-分析系统调用","children":[]}]},{"level":2,"title":"目标文件分析","slug":"目标文件分析","link":"#目标文件分析","children":[{"level":3,"title":"nm","slug":"nm","link":"#nm","children":[]},{"level":3,"title":"objdump","slug":"objdump","link":"#objdump","children":[]},{"level":3,"title":"readelf","slug":"readelf","link":"#readelf","children":[]},{"level":3,"title":"size 查看程序内存占用","slug":"size-查看程序内存占用","link":"#size-查看程序内存占用","children":[]},{"level":3,"title":"file 文件类型查询","slug":"file-文件类型查询","link":"#file-文件类型查询","children":[]},{"level":3,"title":"strings 查询数据中的文本信息","slug":"strings-查询数据中的文本信息","link":"#strings-查询数据中的文本信息","children":[]},{"level":3,"title":"fuser 显示文件使用者","slug":"fuser-显示文件使用者","link":"#fuser-显示文件使用者","children":[]},{"level":3,"title":"xxd 十六进制显示数据","slug":"xxd-十六进制显示数据","link":"#xxd-十六进制显示数据","children":[]},{"level":3,"title":"od","slug":"od","link":"#od","children":[]}]}],"git":{"createdTime":1653565176000,"updatedTime":1684063849000,"contributors":[{"name":"yzqdev","email":"yzqdev@outlook.com","commits":1}]},"readingTime":{"minutes":10.69,"words":3208},"filePathRelative":"linux-tutor/common/program_debug.md","localizedDate":"2022年5月26日","autoDesc":true}');export{c as comp,r as data};
