import{_ as e,c as n,o as t,d as i}from"./app-CbULZrmi.js";const a={},r=i(`<h1 id="readelf-elf文件格式分析" tabindex="-1"><a class="header-anchor" href="#readelf-elf文件格式分析"><span>readelf elf文件格式分析</span></a></h1><p>这个工具和objdump命令提供的功能类似，但是它显示的信息更为具体，并且它不依赖BFD库(BFD库是一个GNU项目，它的目标就是希望通过一种统一的接口来处理不同的目标文件）；</p><p><strong>ELF文件类型</strong></p><p>: ELF(Executable and Linking Format)是一种对象文件的格式，用于定义不同类型的对象文件(Object files)中都放了什么东西、以及都以什么样的格式去放这些东西。它自最早在 System V 系统上出现后，被 xNIX 世界所广泛接受，作为缺省的二进制文件格式来使用。可以说，ELF是构成众多xNIX系统的基础之一。</p><p>ELF文件有三种类型：</p><ol><li></li></ol><pre><code>可重定位的对象文件(Relocatable file)
