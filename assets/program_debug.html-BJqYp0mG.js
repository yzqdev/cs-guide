import{_ as n,c as e,a,o as l}from"./app-Die-Lcms.js";const i={};function p(d,s){return l(),e("div",null,s[0]||(s[0]=[a(`<h1 id="程序调试" tabindex="-1"><a class="header-anchor" href="#程序调试"><span>程序调试</span></a></h1><h2 id="进程调试" tabindex="-1"><a class="header-anchor" href="#进程调试"><span>进程调试</span></a></h2><h3 id="gdb-程序交互调试" tabindex="-1"><a class="header-anchor" href="#gdb-程序交互调试"><span>gdb 程序交互调试</span></a></h3><p>GDB是一个由GNU开源组织发布的、UNIX/LINUX操作系统下的、基于命令行的、功能强大的程序调试工具。</p><p>对于一名Linux下工作的c++程序员，gdb是必不可少的工具；</p><p>GDB中的命令固然很多，但我们只需掌握其中十个左右的命令，就大致可以完成日常的基本的程序调试工作。</p><p>以下从一个完整的调试过程简单说明最基本的几个命令;</p><div class="language-bash" data-highlighter="prismjs" data-ext="sh" data-title="sh"><pre><code><span class="line">    <span class="token variable">$gdb</span> programmer     <span class="token comment"># 启动gdb</span></span>
...
<span class="line"></span></code></pre></div><h3 id="strace-分析系统调用" tabindex="-1"><a class="header-anchor" href="#strace-分析系统调用"><span>strace 分析系统调用</span></a></h3><p>strace常用来跟踪进程执行时的系统调用和所接收的信号。在Linux世界，进程不能直接访问硬件设备，当进程需要访问硬件设备(比如读取磁盘文件，接收网络数据等等)时，必须由用户态模式切换至内核态模式，通过系统调用访问硬件设备。strace可以跟踪到一个进程产生的系统调用,包括参数，返回值，执行消耗的时间。</p><p>完整程序:</p><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    strace -o output.txt -T -tt -e trace=all -p 28979</span>
<span class="line"></span></code></pre></div><p>跟踪28979进程的所有系统调用（-e trace=all），并统计系统调用的花费时间，以及开始时间（以可视化的时分秒格式显示），最后将记录结果存在output.txt文件里面。</p><p>查看进程正在做什么(实时输出进程执行系统调用的情况):</p><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $strace -p &lt;process-pid&gt;</span>
<span class="line"></span></code></pre></div><p>关于strace的详细介绍，详见 <code>strace</code>{.interpreted-text role=&quot;ref&quot;} ;</p><h2 id="目标文件分析" tabindex="-1"><a class="header-anchor" href="#目标文件分析"><span>目标文件分析</span></a></h2><h3 id="nm" tabindex="-1"><a class="header-anchor" href="#nm"><span>nm</span></a></h3><p>nm用来列出目标文件的符号清单。</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $nm myProgrammer</span>
...
<span class="line"></span></code></pre></div><p>也可以查看Core文件是由哪个程序生成:</p><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $file core.22355</span>
<span class="line"></span></code></pre></div><h3 id="strings-查询数据中的文本信息" tabindex="-1"><a class="header-anchor" href="#strings-查询数据中的文本信息"><span>strings 查询数据中的文本信息</span></a></h3><p>一个文件中包含二进制数据和文本数据，如果只需要查看其文本信息，使用这个命令就很方便；过滤掉非字符数据，将文本信息输出:</p><div class="language-text" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">    $strings &lt;objfile&gt;</span>
