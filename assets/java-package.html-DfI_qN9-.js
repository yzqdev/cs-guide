import{_ as t,c as e,o,d as n}from"./app-CbULZrmi.js";const a={},s=n(`<h1 id="java打包的shell" tabindex="-1"><a class="header-anchor" href="#java打包的shell"><span>java打包的shell</span></a></h1><h2 id="注意一个问题" tabindex="-1"><a class="header-anchor" href="#注意一个问题"><span>注意一个问题</span></a></h2><p>由于在DOS（windows系统）下，文本文件的换行符为CRLF，而在Linux下换行符为LF，使用git进行代码管理时，git会自动进行CRLF和LF之间的转换，这个我们不用操心。而有时候，我们需要将windows下的文件上传到linux上，例如shell脚本，执行的时候有时会出现奇怪的问题，这时候，就需要安装dos2unix软件。 ​</p><p>在centos下安装dos2unix：</p><pre><code class="language-shell">yum install -y dos2unix

</code></pre><p>安装完成后，对文件进行转换</p><pre><code class="language-shell">dos2unix xx.sh

</code></pre><p>​</p><p>或者把文本在vscode中编辑,再放回linux</p><h2 id="服务端运行jar包" tabindex="-1"><a class="header-anchor" href="#服务端运行jar包"><span>服务端运行jar包</span></a></h2><pre><code class="language-java">nohup java -jar project.jar &gt;&gt; log.out 2&gt;&amp;1 &amp;
</code></pre><p><code>java -jar XXX.jar &amp;</code> 命令结尾没有 “&amp;” ，则变成 “java -jar XXX.jar ” ，表示在当前ssh窗口，可按CTRL + C打断程序运行，或者直接关闭窗口，则程序直接退出</p><p>命令结尾添加 “&amp;” ，则变成 “java -jar XXX.jar &amp;” ，表示在当窗口关闭时，程序才会中止运行。&amp;代表让该命令在后台执行。</p><p><code>nohup java -jar XXX.jar &gt; Log.log &amp;</code> 或者 <code>nohup java -jar XXX.jar &gt;&gt; Log.log &amp;</code> 命令 &quot;nohup java -jar XXX.jar &amp;&quot; 部分，表示不挂断运行命令,当账户退出或终端关闭时,程序仍然运行。注意，该作业的所有输出被重定向到nohup.out的文件中。</p><p>命令 <code>nohup java -jar XXX.jar &gt; Log.log &amp;</code> 部分，表示不挂断运行命令,当账户退出或终端关闭时,程序仍然运行，并且该作业的所有输出被重定向到Log.log的文件 中。 <code>&gt; Log.log</code> 该命令就是指定日志输出的文件。 <code>&gt;&gt;</code>表示将输出以追加的方式重定向到Log.log中。</p><pre><code class="language-bash">nohup java -jar XXX.jar &gt; Log.log 2&gt;&amp;1 &amp;
</code></pre><p>或者</p><pre><code class="language-bash">nohup java -jar XXX.jar &gt;&gt; Log.log 2&gt;&amp;1 &amp;
</code></pre><p>或者</p><pre><code class="language-bash">nohup java -jar XXX.jar &gt; /dev/null 2&gt;&amp;1 &amp;
</code></pre><p>标准输入文件(stdin)：stdin的文件描述符为0，Unix程序默认从stdin读取数据。 标准输出文件(stdout)：stdout 的文件描述符为1，Unix程序默认向stdout输出数据。 标准错误文件(stderr)：stderr的文件描述符为2，Unix程序会向stderr流中写入错误信息。 屏蔽输出，起到禁止输出作用：/dev/null 是一个特殊的文件，写入到它的内容都会被丢弃；如果尝试从该文件读取内容，那么什么也读不到。但是 /dev/null 文件非常有用，将命令的输出重定向到它，会起到&quot;禁止输出&quot;的效果。 <code>&gt; Log.log 2&gt;&amp;1</code> ：表示将 stdout 和 stderr 合并后重定向到 Log.log 备注：输出之后，可以使用“jobs”查看一下后台运行的任务。</p><h2 id="下面是一个bash命令" tabindex="-1"><a class="header-anchor" href="#下面是一个bash命令"><span>下面是一个bash命令</span></a></h2><pre><code class="language-bash">#!/bin/bash
# 定义变量
...
</code></pre><h2 id="运行go文件的命令" tabindex="-1"><a class="header-anchor" href="#运行go文件的命令"><span>运行go文件的命令</span></a></h2><pre><code class="language-shell">#!/bin/bash
# 定义变量
# 要运行的jar包路径，加不加引号都行。 注意：等号两边 不能 有空格，不然会提示command找不到
