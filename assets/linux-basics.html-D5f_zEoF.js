...
关于安全方面的细节可以参考 apt-secure(8).
                                         本 APT 具有超级牛力。
</code></pre><p>好玩的软件 oneko,cmatrix,</p><p>软件安装,</p><div class="hint-container tip"><p class="hint-container-title">提示</p><p>gdebi ,gedit ,cinnamon ,mate,xfce,unity,gnome-fontviewer,或者 font-manager,gnome-ternimal,firefox,screefetch,file-roller(压缩包管理器),tree,transmission(迅雷),gnome-software 软件更新器,gnome-packagekit 软件包 unbuntu-desktop, Software-properties-gtk(软件源,ppa 更新) Gnome-software (软件商店) Gnome-systemmoniter(任务管理器) FTP/SFTP 客户端: 声音视频:audacity,vlc,rhythmbox 图像处理:gimp 抓图工具:shutter gnome-web-photo Mypaint,darktable,inkscape,dia,shotwell, 录屏工具:gek-recordmydesktop 视频后期处理:openshot</p></div><p>搜狗输入法安装,安装 gdebi 进入搜狗输入法目录输入命令行: <code>sudo gdebi sogoupinyin_2.1.0.0086_amd64.deb</code> 安装 cinnamon :<code>sudo apt install cinnamon</code> 然后运行<code>cinnamon –replace</code> 运行图形化命令,gksu Filezilla:<code>sudo aptitude install –y filezilla</code></p><p>安装主题:</p><ol><li>elementary os 的主题</li></ol><pre><code class="language-shell">sudo add-apt-repository ppa:elementary-os/daily
sudo apt-get update
sudo apt-get install elementary-icon-theme
</code></pre><ol start="2"><li>ubuntukylin ukui 主题, www.ukui.org 官网教程安装这个就好</li><li>zorinos 主题安装方法，首先打开终端输入以下命令安装主题：</li></ol><pre><code class="language-shell">sudo add-apt-repository ppa:noobslab/themes
sudo apt-get update
sudo apt-get install zorinos-themes
</code></pre><p>在终端输入以下命令安装图标主题：</p><pre><code class="language-shell">sudo add-apt-repository ppa:noobslab/icons2
sudo apt-get update
sudo apt-get install zorinos-icons
</code></pre><p>安装或使用调节工具：如 Unity Tweak Tool、Gnome-tweak-tool 或者 Ubuntu-Tweak 来启用主题和图标。</p><h2 id="第二章-linux-的文件系统" tabindex="-1"><a class="header-anchor" href="#第二章-linux-的文件系统"><span>第二章 linux 的文件系统</span></a></h2><p>Lib 各种动态链接库存放的位置,类似 windows 的 dll 文件,只不过 linux 的是.so 文件,</p><pre><code>└─$ tree -d -L 1/
.
├── bin -&gt; usr/bin
├── boot
├── dev
